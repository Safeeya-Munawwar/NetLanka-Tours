  ...
  marginTop: "0",  }}>
      ...
      <div style={containerStyle}>
        {!selectedDestination ? (
          ...
          <>
            <button onClick={() => setSelectedDestination(null)} style={{ ...bookButton, width: 80 }}>← Back</button>
                ...
                {filteredTours.map((tour) => (
                  <SwiperSlide key={tour._id}>
                    <div style={cardStyle}>
                      ...
                      <p style={infoStyle}>⭐ {Array.from({ length: 5 }, (_, i) => (<span key={i} style={{ color: i < tour.rating ? "#ffc107" : "#ccc" }}>★</span>))}</p>
                      ...
                      {tour.itinerary?.length > 0 && (
                        <p onClick={() => toggleItinerary(tour._id)} style={itineraryLinkStyle}>
                          Itinerary <span style={arrowStyle(itineraryVisibility[tour._id])}>▶</span>
                        </p>
                      )}
